...
読み込むウェイトは実際に使っている 300 と 400 のみ。日本語のWebフォントCSSは巨大で
1ウェイトあたり約115KBあるため、`--fw-medium`/`--fw-bold` を使い始めるときに
`BaseLayout.astro` のリンクへ足す。現状 Zen Kaku Gothic Antique に 200 は無い
（`--fw-thin` は定義のみで未使用）。
