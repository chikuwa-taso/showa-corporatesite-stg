...
フッターの2つのCTAはヘッダーと同じガター（`--header-gutter`）を持ち、その内側を
ちょうど埋める。ボタンと間隔はすべて em で、その em を幅から逆算している
（`--footer-cta-size`）。`--footer-cta-span: 27.6` は現在の2ラベルを em で測った
合計なので、**ラベルを変えたらこの値を取り直すこと**。上限を設けていないため
画面が広いほど文字が大きくなる（1440で約50px、1920で約67px、2560で約90px）。
