...
フッターはヘッダーと対になる帯。上下の余白は同じトークン（`--header-pad-top` /
`--header-pad-bottom`）を共有し、左右も同じ `--header-gutter`。ヘッダー側は
モバイル指定も含めてこのトークンを使うので、両者は幅によらず自動で揃う。
その浅い帯に収まるよう、CTAは本文より一段小さい `--footer-cta-size` で組んでいる。
